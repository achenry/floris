...
yaw_angles = [0] * len(fi.floris.farm.turbines)
ais = [0.33] * len(fi.floris.farm.turbines)
powers = []
true_powers = []
turbine_velocities = []
hor_planes = []
wind_vectors = []
iterations = []
turbine_wind_speeds = [[] for t in range(len(fi.floris.farm.turbines))]
turbine_powers = [[] for t in range(len(fi.floris.farm.turbines))]
...
if test_varying_ws:
    ...
    # 8m/s for first 100 units of time, then 10, then 10 for remainder
    wind_speeds_base = [8]*int(100/dt) + [10]*int(100 / dt) + [10]*int((total_time-200) / dt)
else:
    wind_speeds_base = [8]*int(total_time / dt)

if test_varying_wd:
    wind_dirs_base = [start_wind_dir]*int(100/dt) + [start_wind_dir]*int(100/dt) + [start_wind_dir-20]*int((total_time-200) / dt)
else:
    wind_dirs_base = [start_wind_dir]*int(total_time / dt)
    
for sim_time, ws, wd in zip(np.arange(0, total_time, dt), wind_speeds_base, wind_dirs_base):
...
for i, turbine_power in enumerate(turbine_powers):
    label = "Turbine " + str(i)
    plt.plot(list(np.arange(0, total_time, dt)), turbine_power, label=label)
plt.ylabel("Power (MW)")
plt.xlabel("Time (s)")
plt.legend()
...
for i, turbine_wind_speed in enumerate(turbine_wind_speeds):
    label = "Turbine " + str(i)
    plt.plot(list(np.arange(0, total_time, dt)), turbine_wind_speed, label=label)
...
plt.plot(list(np.arange(0, total_time, dt)), powers, label="Dynamic")
plt.plot(list(np.arange(0, total_time, dt)), true_powers, label="Steady-State")
plt.legend()
plt.xlabel("Time (s)")
plt.ylabel("Power (MW)")

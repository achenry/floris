...
def save_case_matrix(matrix_out, change_vars, dir_matrix):
	...
	col_len = [max([len(str(val)) for val in matrix_out[:, j]] + [len(change_vars[j][header_i]) for header_i in
	                                                              range(n_header_lines)]) for j in
	           range(len(change_vars))]
	
	text_out = []
	for header_i in range(n_header_lines):
		text_out.append(
			''.join([val.center(col + 2) for val, col in zip([var[header_i] for var in change_vars], col_len)]) + '\n')
...
def convert_str(val):
	...
	if try_list(val):
		if try_type(val.squeeze()[0], int) and int(val.squeeze()[0]) == float(val.squeeze()[0]):
			return np.asarray(val, int)
		elif try_type(val.squeeze()[0], float):
			return np.asarray(val, float)
	elif try_type(val, int) and int(val) == float(val):
		return int(val)

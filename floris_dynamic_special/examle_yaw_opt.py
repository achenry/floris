...
from floridyn.tools.optimization.yaw_optimization.yaw_optimizer_sr import (
    YawOptimizationSR,
)
...
def calculate_aep(fi, df_windrose, column_name="farm_power"):
    ...
    wd_array = list(np.array(df_windrose["wd"].unique(), dtype=float))
    ws_array = list(np.array(df_windrose["ws"].unique(), dtype=float))
    yaw_angles = list(np.array(df_windrose[yaw_cols], dtype=float))
    fi.reinitialize_flow_field(wind_direction=wd_array, wind_speed=ws_array)
